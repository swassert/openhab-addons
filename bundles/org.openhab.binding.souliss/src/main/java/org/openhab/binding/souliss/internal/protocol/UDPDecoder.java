                    ...
                    if (((SoulissGenericHandler) handler).getNode() == tgtnode) {
                        ...
                        switch (sUIDArray[1]) {
                            ...
                            case SoulissBindingConstants.T52:
                                logger.debug("Decoding T52 packet");
                                logger.debug("Float value: {}", getFloatAtSlot(mac, slot));
                                if (!Float.isNaN(getFloatAtSlot(mac, slot))) {
                                    ((SoulissT52Handler) handler).setFloatValue(getFloatAtSlot(mac, slot));
                                }
                                break;

                            case SoulissBindingConstants.T53:
                                logger.debug("Decoding T53 packet");
                                logger.debug("Float value: {}", getFloatAtSlot(mac, slot));
                                if (!Float.isNaN(getFloatAtSlot(mac, slot))) {
                                    ((SoulissT53Handler) handler).setFloatValue(getFloatAtSlot(mac, slot));
                                }
                                break;
